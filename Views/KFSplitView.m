...
- initWithFrame:(NSRect)frameRect{
	if(self = [super initWithFrame:frameRect]) [self kfSetup];

	return self;
}

- initWithCoder:(NSCoder *)coder{
	if(self = [super initWithCoder:coder]) [self kfSetup];
...
- (void)kfSetupResizeCursors{
	...
	if(isVerticalImage){
		kfIsVerticalResizeCursor = [[NSCursor alloc] initWithImage:isVerticalImage hotSpot:NSMakePoint(8,8)];
	}
	...
	if (isNotVerticalImage){
		kfNotIsVerticalResizeCursor = [[NSCursor alloc] initWithImage:isNotVerticalImage hotSpot:NSMakePoint(8,8)];
	}
...
- (void)dealloc{
	[kfInUsePositionNames release];	// ADDED MDC
...
- (BOOL)setPositionAutosaveName:(NSString *)name{
	...
	if([kfInUsePositionNames containsObject:name]) return NO;
