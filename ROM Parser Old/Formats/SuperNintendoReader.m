...
//-----------------------------------------
	...
	NSString *romDeterminedChecksum;	// returns: determinedChecksum		these 2 need switched, and this impimented
...
//-----------------------------------------
	...
	mbit = wholeFileSize / 131072;						//	NSLog(@"mbit: %i", (wholeFileSize / 131072) );
...
//-----------------------------------------
	...
	romHeaderChecksum = [NSString stringWithFormat:@"%08x", ROMHeader.checksum];
...
//		case 0x11: romCartType = @"ROM+RAM";								break;	// ie: Mega Man X 2 (E)(NG-Dump Known)???
	...
	if(fileCRC == 0x3c89322a) romCartType = @"Tri-Star BIOS";	// Tri-Star Dos - NES-SNES (Adaptor Bios).smc
	...
	switch(ROMHeader.MakerCodeA){
		case 0x30:
			switch(ROMHeader.MakerCodeB){
				case 0x30:	romManufacture = @"Capcom";							break; // should be rported as bad, but SFA2 is Capcom
